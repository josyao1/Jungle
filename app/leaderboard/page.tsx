...
import { BETTORS, GAMES, Player } from '@/lib/constants'
            ...
            {scores.map((entry, i) => (
              <>
                ...
                >
                  ...
                  <td className="capitalize font-medium">{entry.player}</td>
                  <td className="text-center stat-positive font-medium">{entry.totalCorrectPicks}</td>
                  <td className="text-center stat-negative font-medium">{entry.totalMissedPicks}</td>
                ...
                {expandedPlayer === entry.player && entry.games.length > 0 && (
                  <tr key={`${entry.player}-expanded`}>
                    <td colSpan={5} className="bg-white/[0.02] px-3 md:px-6 py-4">
